...
# FOR THE PHASE 4 ULTIMATE HYBRID BENCHMARK EVALUATION TABLE
# ==============================================================================
...
from torch_geometric.nn import SAGEConv, JumpingKnowledge, GatedGraphConv
...
def load_pyg_dataset(csv_path, edge_csv_path):
    ...
    # LSTM Aggregator requires edge_index to be sorted by destination
...
class UltimateHybridGNN(torch.nn.Module):
    def __init__(self, num_features):
        super(UltimateHybridGNN, self).__init__()
        
        # 1. Expand features to fixed size
        self.embedding = torch.nn.Linear(num_features, 64)
        self.bn_emb = torch.nn.LayerNorm(64) # LayerNorm stabilizes RNN gradients
        
        # 2. GNN + RNN Hybrid Layers (GatedGraphConv = GGNN)
        # We manually unroll the GGNN to 1-layer steps so we can extract the memory states!
        self.conv1 = GatedGraphConv(64, num_layers=1)
        self.bn1 = torch.nn.LayerNorm(64)
        
        self.conv2 = GatedGraphConv(64, num_layers=1)
        self.bn2 = torch.nn.LayerNorm(64)
        
        self.conv3 = GatedGraphConv(64, num_layers=1)
        self.bn3 = torch.nn.LayerNorm(64)
        
        # 3. Anti-Oversmoothing: Jumping Knowledge Network
        # 'max' pooling will take the strongest Trojan signals from ALL layers
    ...
    def forward(self, x, edge_index):
        ...
        # Jumping Knowledge: Combine all GGNN steps to prevent oversmoothing
...
def train_ultimate(train_graphs):
    loader = DataLoader(train_graphs, batch_size=1, shuffle=True)
    
    model = UltimateHybridGNN(num_features=8).to(device)
    ...
    print(f"\n--- Training Ultimate Hybrid Model (GNN+RNN+JK) on {len(train_graphs)} graphs ---")
...
if __name__ == "__main__":
    ...
    print("\n==================================================")
    print("FINAL EVALUATION TABLE (ULTIMATE HYBRID MODEL)")
    print("==================================================")
    ...
    global_model = train_ultimate(global_train_graphs)
    global_model.eval()
    ...
    results_df.to_csv('evaluation_results_ultimate.csv', index=False)
    
    print("\nEVALUATION COMPLETE.")
    print("The table has also been saved to 'evaluation_results_ultimate.csv'.")
