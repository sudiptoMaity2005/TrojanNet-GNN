...
class AdvancedGNN(torch.nn.Module):
    def __init__(self, num_features):
        ...
        # GraphSAGE often performs much better than GCN on tabular node features
...
def train_gnn(train_graphs):
    ...
    # Tune the weights. 50x penalty is too aggressive and kills precision.
    # We will cap the Trojan weight at 15.0 to balance Precision and Recall.
    weight_0 = 1.0
    weight_1 = min(total_negatives / max(total_positives, 1), 15.0) 
    
    class_weights = torch.tensor([weight_0, weight_1], dtype=torch.float).to(device)
    
    # NEW: Using Focal Loss instead of Cross Entropy to force learning on hard sequential nodes
    criterion = FocalLoss(alpha=class_weights, gamma=2.0)
    
    print(f"\n--- Training Advanced GraphSAGE on {len(train_graphs)} graphs ---")
    print(f"Dataset Imbalance: {total_negatives} Clean vs {total_positives} Trojans")
    print(f"Applying Tuned Class Weights: Class 0: {weight_0:.2f}, Class 1: {weight_1:.2f}")
...
if __name__ == "__main__":
    ...
    print("\n==================================================")
    print("FINAL EVALUATION TABLE (INTRA-FAMILY)")
    print("==================================================")
    ...
    for circ_name, data_dict in circuit_families.items():
        ...
        # Train a specific model just for this circuit family
        model = train_gnn(train_graphs)
        model.eval()
